...
int parent_process_read(char *filename) // 부모 프로세스 (파일 읽기 역할만 수행)
{
	int shmid, len;
	void *shmaddr;
...
int child_process_write(char *filename) // 자식 프로세스 (파일 쓰기 역할만 수행)
{
	int shmid, len; 
	void *shmaddr;
...
int main(int argc, char **argv)
{
	...
	if (argc != 3)
	{
        fprintf(stderr, "사용 방법은 : %s <orignial file name> <create file name>\n", argv[0]);
        exit(1);
	}
